...
int_T main(void)
{
  ...
  init();
  RetractExpandCommand_initialize();
  ...
  /* The main step loop */
  while (rtmGetErrorStatus(RetractExpandCommand_M) == (NULL)) {
    actualtime = micros();
    if ((unsigned long) (actualtime - oldtime) >= STEP_SIZE) {
      oldtime = actualtime;
      RetractExpandCommand_output();

      /* Get model outputs here */
      RetractExpandCommand_update();
    }
  }

  RetractExpandCommand_terminate();
  return 0;
}
